...
- **🚀 完全自包含**：地图库 Leaflet 已本地化，**不依赖任何国外 CDN**，国内网络下稳定加载。底图采用**高德地图（AutoNavi）**瓦片——OSM / CartoDB 等境外瓦片源在中国大陆被墙无法加载，高德瓦片国内直连、免 Key；由于高德采用 GCJ-02 坐标，应用已对全部台风轨迹数据做 WGS-84→GCJ-02 转换以与底图精确对齐。
